...
def planar_cycles(undirected_graph):
    def angle(v1, v2, relative = False):
        ...
        if relative == False and x[0] * y[1] < y[0] * x[1]:
    ...
    def is_chord_free(_graph, _cycle):
        ...
        for i in range(0, len(_cycle) - 1):
            current_angle = angle(_cycle[i - 1] - _cycle[i - 2], _cycle[i] - _cycle[i - 1])
        ...
        for i in range(1, len(_cycle) - 2):
            angle_to_next_node = angle(_cycle[i - 1] - _cycle[i], _cycle[i + 1] - _cycle[i]) 
            neighbors = _graph.neighbors(_cycle[i])
            for n in neighbors:
                if n != cycle[i + 1] and n != cycle[i - 1]:
                    angle_to_neighbor = angle(_cycle[i - 1] - _cycle[i], n - _cycle[i]) 
