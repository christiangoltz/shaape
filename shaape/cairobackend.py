...
class CairoBackend(DrawingBackend):
    ...
    def blur_surface(self):
        ...
        dst[:,:,0] = ndimage.gaussian_filter(src[:,:,0], sigma=5)
        dst[:,:,1] = ndimage.gaussian_filter(src[:,:,1], sigma=5)
        dst[:,:,2] = ndimage.gaussian_filter(src[:,:,2], sigma=5)
        dst[:,:,3] = ndimage.gaussian_filter(src[:,:,3], sigma=5)
        dst[:,:,3] = dst[:,:,3] * 0.3
